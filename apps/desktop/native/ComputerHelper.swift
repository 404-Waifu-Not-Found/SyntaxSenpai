...
func releaseInputs() {
    ...
    for key: CGKeyCode in [54,55,56,58,59,60,61,62] { CGEvent(keyboardEventSource: source, virtualKey: key, keyDown: false)?.post(tap: .cghidEventTap) }
    ...
    CGEvent(mouseEventSource: source, mouseType: .rightMouseUp, mouseCursorPosition: p, mouseButton: .right)?.post(tap: .cghidEventTap)
}
func observe(_ args: [String: Any]) throws -> [String: Any] {
    try trusted()
    let apps = NSWorkspace.shared.runningApplications.filter { $0.activationPolicy == .regular }
    let requested = args["app_id"] as? String
    let app = requested == nil ? NSWorkspace.shared.frontmostApplication : apps.first { $0.bundleIdentifier == requested }
...
func action(_ name: String, _ args: [String: Any]) throws -> [String: Any] {
    if name == "status" { return ["accessibility": AXIsProcessTrusted(), "helperPath": CommandLine.arguments[0]] }
    if name == "setup" { let options = [kAXTrustedCheckOptionPrompt.takeUnretainedValue() as String: true] as CFDictionary; return ["accessibility": AXIsProcessTrustedWithOptions(options)] }
    ...
    if name == "focus" {
        guard let appId = args["app_id"] as? String, let app = NSWorkspace.shared.runningApplications.first(where: { $0.bundleIdentifier == appId }) else { throw NSError(domain: "Unknown application", code: 4) }
        app.activate(options: [.activateAllWindows, .activateIgnoringOtherApps])
        observationID = ""; return ["success": true, "action": name]
    }
    guard NSWorkspace.shared.frontmostApplication?.processIdentifier == observedPID else { throw NSError(domain: "Focused application changed. Observe again.", code: 5) }
    if let expected = observedWindow {
        let root = AXUIElementCreateApplication(observedPID)
        guard let current = attr(root, kAXFocusedWindowAttribute), CFEqual(current, expected) else { throw NSError(domain: "Focused window changed. Observe again.", code: 6) }
    }
    let source = CGEventSource(stateID: .hidSystemState)
    func mouse(_ type: CGEventType, _ point: CGPoint, _ button: CGMouseButton = .left, _ count: Int64 = 1) { let e = CGEvent(mouseEventSource: source, mouseType: type, mouseCursorPosition: point, mouseButton: button); e?.setIntegerValueField(.mouseEventClickState, value: count); e?.post(tap: .cghidEventTap) }
    ...
    case "type":
        ...
        var chunk = ""
        func post(_ value: String) { let utf = Array(value.utf16); for down in [true, false] { let e = CGEvent(keyboardEventSource: source, virtualKey: 0, keyDown: down); e?.keyboardSetUnicodeString(stringLength: utf.count, unicodeString: utf); e?.post(tap: .cghidEventTap) } }
        for c in text { chunk.append(c); if chunk.utf16.count >= 80 { post(chunk); chunk = "" } }; if !chunk.isEmpty { post(chunk) }
    case "key":
    ...
    case "scroll":
        ...
        CGEvent(scrollWheelEvent2Source: source, units: .pixel, wheelCount: 2, wheel1: Int32(max(-5000, min(5000, args["dy"] as? Int ?? -500))), wheel2: Int32(max(-5000, min(5000, args["dx"] as? Int ?? 0))), wheel3: 0)?.post(tap: .cghidEventTap)
    case "drag":
